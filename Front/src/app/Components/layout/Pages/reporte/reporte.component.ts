...
import 'jspdf-autotable'; // Importa jspdf-autotable para usar autoTable
...
export class ReporteComponent implements OnInit {
  ...
  exportarPDF() {
    ...
    // Configurar las columnas y los datos para la tabla
    const columns = ['Número de Documento', 'Tipo de Pago', 'Fecha de Registro', 'Total de Venta', 'Producto', 'Cantidad', 'Precio', 'Total'];
    const rows = this.listaVentasReporte.map(report => [report.numeroDocumento, report.tipoPago, report.fechaRegistro, report.totalVenta, report.producto, report.cantidad, report.precio, report.total]);

    // Agregar la tabla al PDF
    (doc as any).autoTable({ // Usa 'autoTable' en lugar de 'doc.autoTable' y castea 'doc' a 'any'
      head: [columns],
      body: rows
    });

    // Guardar el PDF con el nombre "Reporte Ventas.pdf"
